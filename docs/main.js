...
function initialiseSVG() {
  ...
  let xScale = d3.scaleBand().range([0, chartWidth]).padding(0.1);
}

// Load data function adapted from hw
...
function raceDeathVis() {
  ...
  if (
    keyframes[keyframeIndex].activeVerse === 2 &&
    keyframes[keyframeIndex].activeLines.includes(5)
  ) {
    defaultDemographic = "Black including Hispanic";
  } else if (
    keyframes[keyframeIndex].activeVerse === 3 &&
    keyframes[keyframeIndex].activeLines.includes(4) &&
    keyframes[keyframeIndex].activeLines.includes(5)
  ) {
  ...
  d3.csv(CSV_FILE_PATH)
    .then(function (data) {
      ...
      // Creating  a group for the main chart area
      const chartArea = svg
        .append("g")
        .attr("transform", `translate(${margin.left}, ${margin.top})`);
      ...
      // Created a tooltip so that when users hover over a state it gives state details
      ...
      // Update chart based on selected demographic
      function updateChart(demographic) {
        ...
        svg
          .select(".title")
          .text(
            `Infant Mortality Rates by County: ${demographic} Population %`
          );
      }
    })
    .catch((error) => {
